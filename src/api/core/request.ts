  ...
  Object.keys(params).forEach((key) => {
    ...
    if (isDefined(paramValue)) {
      if (Array.isArray(paramValue)) {
        ...
        qs.push(`${encodeURIComponent(key)}=${encodeURIComponent(String(paramValue))}`);
      }
    }
  });
  if (qs.length > 0) {
...
async function sendRequest(options: ApiRequestOptions, url: string): Promise<Response> {
  const theRequest: RequestInit = {
    method: options.method,
    headers: await getHeaders(options),
    body: getRequestBody(options),
  };
  if (OpenAPI.WITH_CREDENTIALS) {
    theRequest.credentials = 'include';
  }
  return fetch(url, theRequest);
}
